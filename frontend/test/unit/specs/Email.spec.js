...
describe('Email.vue', () => {
  ...
  it('should handle API response error', async () => {
    mockAxios.get.mockImplementation(() =>
      Promise.resolve({ results: [{ 'outgoing-non-gmail-msgs': { N: '0' }, id: { N: '1577192400' }, 'outgoing-gmail-msgs': { N: '0' }, 'incoming-gmail-msgs': { N: '0' }, 'incoming-non-gmail-msgs': { N: '3' } }] }),
    );

    const storeMetrics = await wrapper.vm.storeMetrics();

    expect(storeMetrics).toHaveResolved();
  });
});
